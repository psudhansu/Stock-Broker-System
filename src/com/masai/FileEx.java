...
public class FileEx {
	public static Map<String, Stocks>  stockFile() {

		Map<String, Stocks>  pFile = null;

		File f = new File("Stock.sdn");
		boolean flag = false;
		try {
			if (!f.exists()) {
				f.createNewFile();
			...
			if (flag) {
				...
				pFile = (Map<String, Stocks>) ois.readObject();
	...
	public static List<Transaction> transactionFile() {
		...
		File f = new File("Transactions.sdn");
		boolean flag = false;
		try {
			if (!f.exists()) {
				f.createNewFile();
